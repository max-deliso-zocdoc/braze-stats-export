...
class CanvasListResponse:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> CanvasListResponse:
        ...
        return cls(canvases=canvases, message=data["message"])
...
class CanvasStepPath:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> CanvasStepPath:
        return cls(name=data.get("name", ""), next_step_id=data.get("next_step_id", ""))
...
class CanvasStep:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> CanvasStep:
        ...
        next_paths = (
            [CanvasStepPath.from_dict(path_data) for path_data in next_paths_data]
            if next_paths_data
            else []
        )
...
class CanvasDetails:
    ...
    def from_dict(cls, data: Dict[str, Any], canvas_id: str = "") -> CanvasDetails:
        ...
        variants = (
            [CanvasVariant.from_dict(variant_data) for variant_data in variants_data]
            if variants_data
            else []
        )
        ...
        steps = (
            [CanvasStep.from_dict(step_data) for step_data in steps_data]
            if steps_data
            else []
        )
